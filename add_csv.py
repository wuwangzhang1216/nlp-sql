...
def main(split, file):
    ...
    csv_to_sqlite(table_id, file, '{}.db'.format(split), working_folder)
    csv_to_json(table_id, file, '{}.tables.jsonl'.format(split), working_folder)
    print(f'Adding table with name {table_id}...')
    # print("Added table with id '{id}' (name '{name}') to {split}.db and {split}.tables.jsonl".format(
    #     id=table_id, name=get_table_name(table_id), split=split))
