...
def main(model_file, bert_model_file, bert_path, data_path, split, result_path):
    parser = argparse.ArgumentParser()
    parser.add_argument("--model_file", type=str, default=model_file)
    parser.add_argument("--bert_model_file", type=str, default=bert_model_file)
    ...
    args = construct_hyper_param(parser=parser)
...
# no_pretraining = True  # counterintuitive, but avoids loading unused models
# model, model_bert, tokenizer, bert_config = get_models(args, BERT_PT_PATH, trained=True, path_model_bert=path_model_bert, path_model=path_model)

# # Load data
# dev_data, dev_table = load_wikisql_data(data_path, mode=split, toy_model=toy_model, toy_size=toy_size, no_hs_tok=True)
# dev_loader = torch.utils.data.DataLoader(
#     batch_size=bS,
#     dataset=dev_data,
#     shuffle=False,
